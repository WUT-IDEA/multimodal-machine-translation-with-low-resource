...
from fairseq.binarizer import AlignmentDatasetBinarizer, FileBinarizer, VocabularyDatasetBinarizer
...
def _build_dictionary(filenames, task, args, src=False, tgt=False):
    assert src ^ tgt
    return task.build_dictionary(filenames, workers=args.workers,
...
def _make_binary_dataset(vocab: Dictionary, input_prefix: str, output_prefix: str,
                         lang: tp.Optional[str], num_workers: int, args: Namespace):
    ...
    final_summary = FileBinarizer.multiprocess_dataset(input_file, args.dataset_impl, binarizer,
                                                       full_output_prefix, vocab_size=len(vocab),
...
def _make_binary_alignment_dataset(input_prefix: str, output_prefix: str, num_workers: int, args: Namespace):
    ...
    final_summary = FileBinarizer.multiprocess_dataset(input_file, args.dataset_impl, binarizer,
                                                       full_output_prefix, vocab_size=None,
                                                       num_workers=num_workers)

    logger.info("[alignments] {}: parsed {} alignments".format(input_file, final_summary.num_seq))
...
def _make_dataset(vocab: Dictionary, input_prefix: str, output_prefix: str, lang: tp.Optional[str],
                  args: Namespace, num_workers: int):
    if args.dataset_impl == "raw":
        # Copy original text file to destination folder
        output_text_file = _dest_path(output_prefix + ".{}-{}".format(args.source_lang, args.target_lang),
                                      lang, args.destdir)
...
def _make_all(lang, vocab, args):
    if args.trainpref:
        _make_dataset(vocab, args.trainpref, "train", lang, args=args, num_workers=args.workers)

    if args.validpref:
        for k, validpref in enumerate(args.validpref.split(",")):
            outprefix = "valid{}".format(k) if k > 0 else "valid"
            _make_dataset(vocab, validpref, outprefix, lang, args=args, num_workers=args.workers)

    if args.testpref:
        for k, testpref in enumerate(args.testpref.split(",")):
            outprefix = "test{}".format(k) if k > 0 else "test"
            _make_dataset(vocab, testpref, outprefix, lang, args=args, num_workers=args.workers)


def _make_all_alignments(args):
    if args.trainpref and os.path.exists(args.trainpref + "." + args.align_suffix):
        _make_binary_alignment_dataset(args.trainpref + "." + args.align_suffix, "train.align",
                                       num_workers=args.workers, args=args)

    if args.validpref and os.path.exists(args.validpref + "." + args.align_suffix):
        _make_binary_alignment_dataset(args.validpref + "." + args.align_suffix, "valid.align",
                                       num_workers=args.workers, args=args)

    if args.testpref and os.path.exists(args.testpref + "." + args.align_suffix):
        _make_binary_alignment_dataset(args.testpref + "." + args.align_suffix, "test.align",
                                       num_workers=args.workers, args=args)
...
def _align_files(args, src_dict, tgt_dict):
    ...
    with open(os.path.join(args.destdir, "alignment.{}-{}.txt".format(args.source_lang, args.target_lang)),
...
def main(args):
    ...
    logger.info(args)
    assert (args.dataset_impl != "huffman"), "preprocessing.py doesn't support Huffman yet, " \
                                             "use HuffmanCodeBuilder directly."
    ...
    if args.joined_dictionary:
        assert (not args.srcdict or not args.tgtdict), "cannot use both --srcdict and --tgtdict with" \
                                                       " --joined-dictionary"
        ...
        if args.srcdict:
            ...
            src_dict = _build_dictionary({_train_path(lang, args.trainpref)
                                          for lang in [args.source_lang, args.target_lang]},
                                         task=task, args=args, src=True)
        ...
        if args.srcdict:
            ...
            assert args.trainpref, "--trainpref must be set if --srcdict is not specified"
            src_dict = _build_dictionary([_train_path(args.source_lang, args.trainpref)],
                                         task=task, args=args, src=True)
        ...
        if target:
            if args.tgtdict:
                ...
                tgt_dict = _build_dictionary([_train_path(args.target_lang, args.trainpref)],
                                             task=task, args=args, tgt=True)
        else:
            tgt_dict = None
