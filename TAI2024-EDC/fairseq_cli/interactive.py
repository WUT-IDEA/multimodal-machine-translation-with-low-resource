...
logging.basicConfig(format="%(asctime)s | %(levelname)s | %(name)s | %(message)s",
                    datefmt="%Y-%m-%d %H:%M:%S", level=os.environ.get("LOGLEVEL", "INFO").upper(),
                    stream=sys.stdout)
...
def make_batches(lines, cfg, task, max_positions, encode_fn):
    ...
    if cfg.generation.constraints:
        ...
        for i, constraint_list in enumerate(batch_constraints):
            batch_constraints[i] = [task.target_dictionary.encode_line(encode_fn_target(constraint),
...
def main(cfg: FairseqConfig):
    ...
    assert (not cfg.dataset.batch_size or cfg.dataset.batch_size <= cfg.interactive.buffer_size), \
    ...
    models, _model_args = checkpoint_utils.load_model_ensemble(utils.split_paths(cfg.common_eval.path),
        ...
        for id_, src_tokens, hypos, info in sorted(results, key=lambda x: x[0]):
            ...
            if src_dict is not None:
                ...
                for constraint in info["constraints"]:
                    print("C-{}\t{}".format(id_, tgt_dict.string(constraint, cfg.common_eval.post_process)))
            ...
            for hypo in hypos[: min(len(hypos), cfg.generation.nbest)]:
                ...
                print("P-{}\t{}".format(id_, " ".join(map(lambda x: "{:.4f}".format(x),
    ...
    logger.info("Total time: {:.3f} seconds; translation time: {:.3f}".format(time.time() - start_time,
