...
def infer_init_method(cfg: DistributedTrainingConfig, force_distributed=False):
    ...
    if cfg.pipeline_model_parallel:
        ...
        with open_dict(cfg):
            cfg.distributed_num_procs = min(torch.cuda.device_count(), cfg.distributed_world_size)
...
def _pipeline_parallel_pre_init(cfg: DistributedTrainingConfig):
    ...
    balance_exists = (cfg.pipeline_balance is not None or cfg.pipeline_encoder_balance is not None
                      or cfg.pipeline_decoder_balance is not None)

    devices_exist = (cfg.pipeline_devices is not None or cfg.pipeline_encoder_devices is not None
    ...
    if cfg.pipeline_devices is not None:
        ...
        cfg.pipeline_encoder_devices = utils.eval_str_list(cfg.pipeline_encoder_devices, type=int)
        cfg.pipeline_decoder_devices = utils.eval_str_list(cfg.pipeline_decoder_devices, type=int)
        num_pipeline_devices = len(set(cfg.pipeline_encoder_devices + cfg.pipeline_decoder_devices))
...
def distributed_init(cfg: FairseqConfig):
    ...
    if not cfg.common.tpu:
        if torch.distributed.is_available() and torch.distributed.is_initialized():
            ...
            logger.info("distributed init (rank {}): {}".format(cfg.distributed_training.distributed_rank,
                                                                cfg.distributed_training.distributed_init_method, ))
            dist.init_process_group(backend=cfg.distributed_training.distributed_backend,
                                    init_method=cfg.distributed_training.distributed_init_method,
                                    world_size=cfg.distributed_training.distributed_world_size,
                                    rank=cfg.distributed_training.distributed_rank)
            logger.info("initialized host {} as rank {}".format(socket.gethostname(),
    ...
    if cfg.common.model_parallel_size > 1:
        try:
            from fairseq.model_parallel.megatron.mpu import (initialize_model_parallel,
...
def call_main(cfg: FairseqConfig, main, **kwargs):
    ...
    if cfg.distributed_training.distributed_init_method is not None:
        ...
        if not cfg.distributed_training.distributed_no_spawn:
            ...
            torch.multiprocessing.spawn(fn=distributed_main, args=(main, cfg, kwargs),
                                        nprocs=min(torch.cuda.device_count(),
                                                   cfg.distributed_training.distributed_world_size, ), join=True, )
        else:
        ...
        xmp.spawn(fn=distributed_main, args=(main, cfg, kwargs),
                  # tpu-comment:
...
def all_to_all(tensor, group):
    ...
    if use_xla():
        assert isinstance(group, tuple) and group[0] == "tpu"
        return xm.all_to_all(tensor, split_dimension=0, concat_dimension=0, split_count=split_count,
                             groups=group[1], )
    else:
...
def all_gather(tensor, group, return_tensor=False):
    ...
    if use_xla():
        ...
        tensor_list = [tensor if i == rank else torch.empty_like(tensor) for i in range(world_size)]
...
def broadcast_tensors(tensors: Optional[List[torch.Tensor]], src_rank: int, group: object,
                      dist_device: Optional[torch.device] = None,) -> List[torch.Tensor]:
    """
...
def _broadcast_object_slow(obj: Any, src_rank: int, group: object, dist_device: torch.device,) -> Any:
    if get_rank(group) == src_rank:
        # Emit data
        buffer = io.BytesIO()
