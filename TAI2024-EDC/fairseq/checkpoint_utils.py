...
def save_checkpoint(cfg: CheckpointConfig, trainer, epoch_itr, val_loss):
    ...
    if val_loss is not None and cfg.keep_best_checkpoints > 0:
        ...
        chkpts = checkpoint_paths(cfg.save_dir, pattern=r"checkpoint\.best_{}_(\d+\.?\d*){}\.pt".format(
        ...
        checkpoint_conds[
            "checkpoint.best_{}_{:.3f}{}{}.pt".format(cfg.best_checkpoint_metric, val_loss, rand_sfx, suffix)
    ...
    if len(checkpoints) > 0 and trainer.should_save_checkpoint_on_current_rank:
        ...
        write_timer.stop()
        logger.info("Saved checkpoint {} (epoch {} @ {} updates, score {}) (writing took {} seconds)".format(
    ...
    if cfg.keep_best_checkpoints > 0:
        ...
        checkpoints = checkpoint_paths(cfg.save_dir, pattern=r"checkpoint\.best_{}_(\d+\.?\d*){}\.pt".format(
            cfg.best_checkpoint_metric, suffix))
...
def load_checkpoint(cfg: CheckpointConfig, trainer, **passthrough_args):
    ...
    *passthrough_args* will be passed through to
    ...
    if cfg.restore_file == "checkpoint_last.pt":  # default value of restore_file is 'checkpoint_last.pt'
        ...
        if first_launch and getattr(cfg, "continue_once", None) is not None:
            ...
            if PathManager.exists(cfg.finetune_from_model):
                ...
                logger.info(f"loading pretrained model from {checkpoint_path}:optimizer, lr scheduler, meters, "
                            f"dataloader will be reset")
            else:
                raise ValueError(f"--funetune-from-model {cfg.finetune_from_model} does not exist")
    ...
    if cfg.restore_file != "checkpoint_last.pt" and cfg.finetune_from_model:
        raise ValueError("--finetune-from-model and --restore-file (non-default value) can not be specified"
                         "together: " + str(cfg))

    extra_state = trainer.load_checkpoint(checkpoint_path, reset_optimizer, reset_lr_scheduler,
...
def load_model_ensemble(filenames, arg_overrides: Optional[Dict[str, Any]] = None, task=None,
                        strict=True, suffix="", num_shards=1, state=None):
...
def load_model_ensemble_and_task(filenames, arg_overrides: Optional[Dict[str, Any]] = None,
                                 task=None, strict=True, suffix="", num_shards=1, state=None):
    ...
    for filename in filenames:
        ...
        for shard_idx in range(num_shards):
            ...
            if "fsdp_metadata" in state and num_shards > 1:
                ...
                if not has_FSDP:
                    raise ImportError("Cannot find FullyShardedDataParallel. Please install fairscale with: "
...
def load_model_ensemble_and_task_from_hf_hub(model_id, cache_dir: Optional[str] = None,
                                             arg_overrides: Optional[Dict[str, Any]] = None, **kwargs: Any):
    try:
        from huggingface_hub import snapshot_download
    except ImportError:
        raise ImportError("You need to install huggingface_hub to use `load_from_hf_hub`. "
    ...
    return load_model_ensemble_and_task([p.as_posix() for p in Path(cache_dir).glob("*.pt")],
...
def prune_state_dict(state_dict, model_cfg: Optional[DictConfig]):
    ...
    for layer_name in state_dict.keys():
        ...
        for pruning_pass in pruning_passes:
            if original_layer_number in pruning_pass["mapping_dict"] and pruning_pass["substitution_regex"
            ].search(layer_name):
                new_layer_number = pruning_pass["mapping_dict"][original_layer_number]
                substitution_match = pruning_pass["substitution_regex"].search(layer_name)
                new_state_key = (layer_name[: substitution_match.start(1)] + new_layer_number
...
def load_pretrained_component_from_model(component: Union[FairseqEncoder, FairseqDecoder],
                                         checkpoint: str, strict: bool = True):
    """
    ...
    if isinstance(component, FairseqEncoder):
        ...
        raise ValueError("component to load must be either a FairseqEncoder or FairseqDecoder. "
                         "Loading other component types are not supported.")
...
def load_ema_from_checkpoint(fpath):
    ...
    with PathManager.open(fpath, "rb") as f:
        ...
        if len(params_dict) == 0:
            raise ValueError(f"Input checkpoint path '{fpath}' does not contain ema model weights, "
                             f"is this model trained with EMA?")
