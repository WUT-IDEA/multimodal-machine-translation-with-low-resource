...
class Trainer(object):
    ...
    This class supports synchronous distributed data parallel training, where multiple workers each have a
    full model replica and gradients are accumulated across workers before each update. We use :class:`
    ~torch.nn.parallel.DistributedDataParallel` to handle communication of the gradients across workers.
    """
    ...
    def __init__(self, cfg: FairseqConfig, task, model, criterion, quantizer=None):
        ...
        if self.is_fsdp:
            ...
            if self.cfg.distributed_training.zero_sharding != "none":
                raise ValueError("FullyShardedDataParallel is not compatible with --zero-sharding "
                                 "option (it's already built in)")
            if max(self.cfg.optimization.update_freq) > 1 and fairscale.__version__ < "0.4.0":
                raise RuntimeError("Please update to fairscale 0.4.0 or newer when combining "
                                   "--update-freq with FullyShardedDataParallel")
        else:
            if (hasattr(self.cfg.distributed_training, "cpu_offload")
    ...
    def use_distributed_wrapper(self) -> bool:
        return (self.data_parallel_world_size > 1 and not self.cfg.optimization.use_bmuf) \
    ...
    def criterion(self):
        if self._wrapped_criterion is None:
            if utils.has_parameters(self._criterion) and self.use_distributed_wrapper:
                self._wrapped_criterion = models.DistributedFairseqModel(self.cfg.distributed_training,
                                                                         self._criterion,
    ...
    def model(self):
        if self._wrapped_model is None:
            if self.use_distributed_wrapper:
                self._wrapped_model = models.DistributedFairseqModel(self.cfg.distributed_training,
                                                                     self._model,
                                                                     process_group=self.data_parallel_process_group,
                                                                     device=self.device)
            else:
    ...
    def _build_optimizer(self):
        ...
        if self.is_fsdp and self.cfg.common.fp16:
            ...
            if self.cuda and torch.cuda.get_device_capability(0)[0] < 7:
                logger.info("NOTE: your device does NOT support faster training with --fp16 or --amp, "
        ...
        if self.cfg.distributed_training.zero_sharding == "os":
            if (self.cfg.common.fp16 and not self.cfg.common.memory_efficient_fp16
                and not self.cfg.common.memory_efficient_bf16) and not self.cfg.common.fp16_no_flatten_grads:
                raise ValueError("ZeRO is incomptabile with fp16 and flattened grads. "
    ...
    def save_checkpoint(self, filename, extra_state):
        ...
        if self.should_save_checkpoint_on_current_rank:
            checkpoint_utils.torch_persistent_save(
                state_dict, filename, async_write=self.cfg.checkpoint.write_checkpoints_asynchronously)
    ...
    def load_checkpoint(self, filename, reset_optimizer=False, reset_lr_scheduler=False,
        ...
        if bexists:
            load_on_all_ranks = (self.cfg.checkpoint.load_checkpoint_on_all_dp_ranks
            ...
            if load_on_all_ranks or self.data_parallel_rank == 0:
                ...
                if (not load_on_all_ranks and self.cfg.distributed_training.zero_sharding == "os"
                        and "last_optimizer_state" in state and is_distributed):
            ...
            try:
                if ("optimizer_history" in state and len(state["optimizer_history"]) > 0
                        and "num_updates" in state["optimizer_history"][-1]):
                    self.model.set_num_updates(state["optimizer_history"][-1]["num_updates"])

                # this is the code related to AdaPrune In short, it removes redundant heads in multi-head attention
                # module based on heads importance provided For more info, please refer to the paper:
                # https://openreview.net/forum?id=_CMSV7FTzGI The idea of prune in mha can be summarized as Fine tune
                # model (e.g. roberta encoder) on a certain datasets with regularization After the model is trained.
                # User could use get_reserve_head_index and _adaptive_prune_heads functions to get the top X heads
                # with most importance. Then user uses the rank to prune a new roberta encoder and save the pruned
                # ckpt manually. User will fine tune the the new roberta encoder via the ckpt saved above To get rid
                # of registering different pruned version of Roberta, I use the argument --mha-heads-to-keep to prune
                # the Roberta model into a pruned version which matches the pruned ckpt.

                if (safe_hasattr(self.model, "args") and safe_hasattr(self.model.args, "mha_heads_to_keep")
                        and self.model.args.mha_heads_to_keep != -1):
                    logger.info(f"Prune model: keep {self.model.args.mha_heads_to_keep} heads for each multihead "
                                f"attention module")
                ...
                # this is the code related to AdaPrune In short, it removes redundant units in feedforward layer in
                # each transformer layer based on importance For more info, please refer to the paper:
                # https://openreview.net/forum?id=_CMSV7FTzGI The idea of prune in ffn can be summarized as Fine tune
                # model (e.g. roberta encoder) on a certain datasets with regularization After the model is trained.
                # User could use _get_fc_rank and _prune_fc_layer functions to get the top X units with most
                # importance. Then user uses the rank to prune a new roberta encoder and save the pruned ckpt
                # manually. User will fine tune the the new roberta encoder via the ckpt saved above To get rid of
                # registering different pruned version of Roberta, I use the argument --ffn-blocks-to-remove to prune
                # the Roberta model into a pruned version which matches the pruned ckpt.

                if (safe_hasattr(self.model, "args") and safe_hasattr(self.model.args, "ffn_blocks_to_remove")
                        and self.model.args.ffn_blocks_to_remove != -1):
                    logger.info(f"Prune model: remove {self.model.args.ffn_blocks_to_remove} ffn blocks for each"
                                f" transformer layer")
                    for layer in self.model.encoder.sentence_encoder.layers:
        ...
        if last_optim_state is not None and not reset_optimizer:
            ...
            assert (last_optim["criterion_name"] == self.get_criterion().__class__.__name__), \
                ...
                "{last_optim['criterion_name']} vs {self.get_criterion().__class__.__name__}"

            assert (last_optim["optimizer_name"] == self.optimizer.__class__.__name__), \
                f"Optimizer does not match; please reset the optimizer (--reset-optimizer)." \
                f" {last_optim['optimizer_name']} vs {self.optimizer.__class__.__name__}"
        ...
        if extra_state is not None:
            ...
            if self.cfg.ema.store_ema:
                if "ema" not in extra_state:
                    logger.warn("EMA not found in checkpoint. But store_ema is True. "
    ...
    def get_train_iterator(self, epoch, combine=True, load_dataset=True, data_selector=None,
        ...
        if load_dataset:
            ...
            self.task.load_dataset(self.cfg.dataset.train_subset, epoch=epoch, combine=combine,
                                   data_selector=data_selector, tpu=self.tpu)
        batch_iterator = self.task.get_batch_iterator(dataset=self.task.dataset(self.cfg.dataset.train_subset),
    ...
    def get_valid_iterator(self, subset, disable_iterator_cache=False):
        ...
        batch_iterator = self.task.get_batch_iterator(dataset=self.task.dataset(subset),
    ...
    def train_step(self, samples, raise_oom=False):
        ...
        for i, sample in enumerate(samples):  # delayed update loop
            ...
            try:
                with maybe_no_sync():
                    ...
                    loss, sample_size_i, logging_output = self.task.train_step(sample=sample, model=self.model,
        ...
        try:
            ...
            with torch.autograd.profiler.record_function("multiply-grads"):
                ...
                numer = (self.data_parallel_world_size if not self.cfg.optimization.use_bmuf
            ...
            # check that grad norms are consistent across workers on tpu check tensor is slow

            if not self.tpu:
                if not self.cfg.optimization.use_bmuf and self.cfg.distributed_training.ddp_backend != "slowmo":
                    self._check_grad_norms(grad_norm)
                if not torch.isfinite(grad_norm).all():
                    # in case of AMP, if gradients are Nan/Inf then optimizer step is still required
            ...
            with NanDetector(self.get_model()):
                for _, sample in enumerate(samples):
                    ...
                    self.task.train_step(sample, self.model, self.criterion, self.optimizer,
                                         self.get_num_updates(), ignore_grad=False, **extra_kwargs)
            raise
        ...
        if not overflow or self.cfg.distributed_training.ddp_backend == "slowmo":
            ...
            if self.tpu:
                ...
                if (self.cuda and self.cfg.common.empty_cache_freq > 0 and
        ...
        if self.cfg.common.fp16 or self.cfg.common.amp:
            metrics.log_scalar("loss_scale", (self.optimizer.scaler.loss_scale if self.cfg.common.fp16
                                              else self.optimizer.scaler.get_scale()), priority=700,
                               round=4, weight=0)
    ...
    def valid_step(self, sample, raise_oom=False):
        ...
        # gather logging outputs from all replicas
        if self.data_parallel_world_size > 1:
            logging_outputs, (sample_size,) = self._aggregate_logging_outputs(logging_outputs, sample_size,
    ...
    def clip_grad_norm(self, clip_norm):
        ...
        should_agg_norm = self.is_fsdp and (self.data_parallel_process_group is not None
    ...
    def _prepare_sample(self, sample, is_dummy=False):
        if sample == "DUMMY":
            raise Exception("Trying to use an uninitialized 'dummy' batch. This usually indicates "
                            "that the total number of batches is smaller than the number of "
                            "participating GPUs. Try reducing the batch size or using fewer GPUs.")

        if sample is None or len(sample) == 0:
            assert (self._dummy_batch is not None and len(self._dummy_batch) > 0), \
    ...
    def _sync_stats(self):
        ...
        if self.data_parallel_world_size == 1:
            ...
            return (self.get_num_updates() + 1) % self.cfg.bmuf.global_sync_iter == 0 and (
                    self.get_num_updates() + 1) > self.cfg.bmuf.warmup_iterations
        else:
            return True
    ...
    def _all_gather_list_sync(self, logging_outputs: List[Dict[str, Any]], *extra_stats_to_sum, ignore=False):
        ...
        if ignore:
            logging_outputs = []
        results = list(zip(*distributed_utils.all_gather_list(
    ...
    def _check_grad_norms(self, grad_norm):
        ...
        if self._grad_norm_buf is not None:
            ...
            def is_consistent(tensor):
                ...
                return ((torch.isfinite(tensor).all() and (max_abs_diff / (tensor[0] + 1e-6) < 1e-6).all())
                        or (self.cfg.common.amp and not torch.isfinite(tensor).all())
                        # in case of amp non-finite grads are fine
                        )

            if not is_consistent(self._grad_norm_buf):
                pretty_detail = "\n".join("rank {:3d} = {:.8f}".format(r, n)
                                          for r, n in enumerate(self._grad_norm_buf.tolist()))
                error_detail = "grad_norm across the workers:\n{}\n".format(pretty_detail)
                # use FloatingPointError to trigger NanDetector
                raise FloatingPointError("Fatal error: gradients are inconsistent between workers. "
                                         "Try --ddp-backend=legacy_ddp. Or are you mixing up different generation"
                                         " of GPUs in training?" + "\n" + "-" * 80 + "\n{}\n".format(error_detail)
                                         + "-" * 80)
    ...
    def _reduce_and_log_stats(self, logging_outputs, sample_size, grad_norm=None):
        ...
        if grad_norm is not None and (not torch.is_tensor(grad_norm) or torch.isfinite(grad_norm)):
            ...
            if self.cfg.optimization.clip_norm > 0:
                metrics.log_scalar("clip", torch.where(grad_norm > self.cfg.optimization.clip_norm,
                                                       grad_norm.new_tensor(100),
                                                       grad_norm.new_tensor(0)), priority=500, round=1, )
        ...
        with metrics.aggregate() as agg:
            ...
            if "loss" not in agg:
                if "loss" not in self._warn_once:
                    ...
                    logger.warning("Criterion.reduce_metrics did not log a 'loss' value, which may break some "
                                   "functionality")
    ...
    def _check_xla_compilation(self):
        ...
        if num_xla_compiles > self._num_xla_compiles:
            logger.warning("XLA compilation detected on device #{}; too many of these can lead "
