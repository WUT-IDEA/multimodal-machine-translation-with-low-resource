...
class Quantizer(object):
    def __init__(self, config_path, max_epoch, max_update):
        ...
        if config_path:
            with open(config_path) as config_file:
                config = quantization_options.parse_config_yaml(yaml.safe_load(config_file))
        else:
        ...
        if max_epoch > 0:
            assert max_epoch % num_iterations == 0, ("for iterative PQ, --max-epoch (={}) must be evenly divisible by "
        ...
        if max_update > 0:
            assert max_update % num_iterations == 0, ("for iterative PQ, --max-update (={}) must be evenly divisible by"
                                                      "len(layers_to_quantize) (={})".format(max_update,
                                                                                             num_iterations))
            self.update_schedule = max_update // num_iterations
        else:
            self.update_schedule = None
        assert (self.epoch_schedule is not None) ^ (self.update_schedule is not None), "for iterative PQ, cannot " \
                                                                                       "specify both --max-update " \
                                                                                       "and --max-epoch"
    ...
    def step(self):
        ...
        logger.info("quantizing model (step={}; layers_to_quantize[step]={})".format(
        ...
        quantized_layers = pq.quantize_model_(self.trainer.get_model(), self.size_tracker, self.layers_to_quantize,
                                              self.block_sizes_config, self.n_centroids_config,
    ...
    def begin_epoch(self, epoch):
        ...
        if ((self.epoch_schedule is not None and epoch > 0 and (epoch - 1) % self.epoch_schedule == 0)
    ...
    def step_update(self, num_updates):
        ...
        if self.update_schedule is not None and num_updates > 0 and num_updates % self.update_schedule == 0:
            self.step()

    def state_dict(self):
        return {"n_centroids_config": self.n_centroids_config, "block_sizes_config": self.block_sizes_config,
                "layers_to_quantize": self.layers_to_quantize, "epoch_schedule": self.epoch_schedule,
                "update_schedule": self.update_schedule, "quantization_step": self.quantization_step}
