...
def setup_task(cfg: FairseqDataclass, **kwargs):
    task = None
    # Returns the task if it exists in the cfg otherwise returns None.
    ...
    assert (task is not None), f"Could not infer task type from {cfg}. Available argparse tasks: " \
                               f"{TASK_REGISTRY.keys()}. Available hydra tasks: {TASK_DATACLASS_REGISTRY.keys()}"
