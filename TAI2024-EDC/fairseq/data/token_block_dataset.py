...
class TokenBlockDataset(FairseqDataset):
    ...
    def __init__(self, dataset, sizes, block_size, pad, eos, break_mode=None, include_targets=False,
                 document_sep_len=1, use_plasma_view=False, split_path=None, plasma_path=None):
        ...
        _sizes, block_to_dataset_index, slice_indices = self._build_slice_indices(sizes, break_mode,
                                                                                  document_sep_len, block_size)

        if use_plasma_view:
            plasma_id = (block_size, document_sep_len, str(break_mode), len(dataset))
            self._slice_indices = plasma_utils.PlasmaView(slice_indices, split_path, (plasma_id, 0),
                                                          plasma_path=plasma_path)
            self._sizes = plasma_utils.PlasmaView(_sizes, split_path, (plasma_id, 1), plasma_path=plasma_path)
            self._block_to_dataset_index = plasma_utils.PlasmaView(block_to_dataset_index, split_path,
                                                                   (plasma_id, 2), plasma_path=plasma_path)
    ...
    def _build_slice_indices(sizes, break_mode, document_sep_len, block_size) -> Tuple[np.ndarray]:
        ...
        try:
            ...
            raise ImportError("Please build Cython components with: `pip install --editable .` "
        ...
        if break_mode == "eos":
            ...
            block_to_dataset_index = np.stack([np.arange(len(sizes)),  # starting index in dataset
    ...
    def prefetch(self, indices):
        self.dataset.prefetch({ds_idx for index in indices for start_ds_idx, _, end_ds_idx in [
            self.block_to_dataset_index[index]]for ds_idx in range(start_ds_idx, end_ds_idx + 1)})
