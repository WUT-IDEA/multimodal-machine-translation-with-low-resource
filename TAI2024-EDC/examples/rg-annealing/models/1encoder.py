...
class RGMMTEncoderBase(FairseqEncoder):
    """
    ...
    def __init__(self, cfg, dictionary, embed_tokens, return_fc=False):
        ...
        # visual attention
        self.img_feature_dim = 4 * embed_dim

        self.region_img_fc = Linear(self.img_feature_dim, embed_dim)
        self.grid_img_fc = Linear(self.img_feature_dim, embed_dim)
        ...
        self.merge_attn = MultiheadAttention(embed_dim, cfg.encoder.attention_heads,
                                             dropout=cfg.attention_dropout, self_attention=True)
        self.gate = nn.Linear(2 * embed_dim, embed_dim)

    def fuse_multimodal_features(self, text, grid_img_feat, region_img_feat):

        #grid_img_feat = self.grid_img_project(grid_img_feat.type_as(text))  # [bsz, 49, dim]

        grid_img_feat = self.grid_img_fc(grid_img_feat.type_as(text))
        grid_img_feat = grid_img_feat.transpose(0, 1)  # [49, bsz, dim]

        #region_img_feat = self.region_img_project(region_img_feat.type_as(text))  # [bsz, img_len, dim]
        region_img_feat = self.region_img_fc(region_img_feat.type_as(text))
        ...
        merge, _ = self.merge_attn(query=text, key=img_feat, value=img_feat, key_padding_mask=region_img_mask)
        concat = torch.cat((merge, text), dim=-1)
        gated = torch.sigmoid(self.gate(concat))

        fused = (1 - gated) * text + gated * merge
    ...
    def forward_scriptable(self, src_tokens, src_lengths: Optional[torch.Tensor] = None,
        ...
        # visual attention
        fuse = self.fuse_multimodal_features(x, grid_img_features, region_img_features)
        x = fuse
...
class RGMMTEncoder(RGMMTEncoderBase):
