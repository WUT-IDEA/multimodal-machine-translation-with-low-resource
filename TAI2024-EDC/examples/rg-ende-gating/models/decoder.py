...
class RGGATEDecoder(TransformerDecoder):
    ...
    def build_decoder_layer(self, args, no_encoder_attn=False):
        return RGGATEDecoderLayer(args, no_encoder_attn)
    ...
    def extract_features_scriptable(self, prev_output_tokens, encoder_out: Optional[EncoderOut] = None,
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _, layer_mask, layer_gate = layer(
                x, encoder_out["encoder_out"][0] if encoder_out is not None else None,
                encoder_out["encoder_padding_mask"][0] if encoder_out is not None else None,
                incremental_state, self_attn_mask=self_attn_mask,
                self_attn_padding_mask=self_attn_padding_mask, need_attn=bool((idx == alignment_layer)),
                need_head_weights=bool((idx == alignment_layer)), attacker=self.attacker[idx],
                loss_type=loss_type)
...
class RGGATEDecoderLayer(TransformerDecoderLayer):
    ...
    def forward(self, x, encoder_out: Optional[torch.Tensor] = None,
                ...
                need_attn: bool = False, need_head_weights: bool = False, attacker=None,
                loss_type: str = 'nmt'):
        """
        Args:
        ...
        if self.encoder_attn is not None:
            ...
            x, attn, gate = self.encoder_attn(query=x, key=encoder_out, value=encoder_out,
...
class AttackMultiheadAttention(MultiheadAttention):
    ...
    def forward(self, query, key: Optional[Tensor], value: Optional[Tensor],
                ...
                need_head_weights: bool = False, mask=None,
