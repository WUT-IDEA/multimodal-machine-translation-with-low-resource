...
RGMMT_ROOT=$ACMMT_ROOT/examples/rg-ende-gating


export CUDA_VISIBLE_DEVICES=0

 fairseq-train ${ACMMT_ROOT}/data_bin/$SRC-$TGT \
        --user-dir ${RGMMT_ROOT} --criterion rggate_criterion \
        --task rggate_translation_task --arch rggate_model --optimizer rggate_adam --adam-betas 0.9,0.98 \
