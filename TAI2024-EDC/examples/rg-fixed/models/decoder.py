from fairseq.models.transformer import base_architecture, TransformerDecoder
...
class RGFIXDecoder(TransformerDecoder):
    ...
    def build_attacker(self, args):
        ...
        self.attacker.extend([Attack(embed_dim=args.decoder_embed_dim, num_heads=args.decoder_attention_heads,
                                     dropout=args.attention_dropout) for _ in range(args.decoder_layers)])

    def build_decoder_layer(self, args, no_encoder_attn=False):
        return RGFIXDecoderLayer(args, no_encoder_attn)

    def forward(self, prev_output_tokens, encoder_out: Optional[EncoderOut] = None,
                ...
                return_all_hiddens: bool = False, loss_type: str = 'nmt'):
        """
        Args:
        ...
        """
        x, extra = self.extract_features(prev_output_tokens, encoder_out=encoder_out,
                                         incremental_state=incremental_state, alignment_layer=alignment_layer,
                                         alignment_heads=alignment_heads, loss_type=loss_type)
        if not features_only:
            x = self.output_layer(x)
        return x, extra

    def extract_features(self, prev_output_tokens, encoder_out: Optional[EncoderOut] = None,
                         incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
                         full_context_alignment: bool = False, alignment_layer: Optional[int] = None,
                         alignment_heads: Optional[int] = None, loss_type: str = 'nmt'):
        return self.extract_features_scriptable(prev_output_tokens, encoder_out, incremental_state,
                                                full_context_alignment, alignment_layer, alignment_heads,
                                                loss_type)

    def extract_features_scriptable(self, prev_output_tokens, encoder_out: Optional[EncoderOut] = None,
                                    incremental_state: Optional[Dict[str, Dict[str, Optional[Tensor]]]] = None,
                                    full_context_alignment: bool = False, alignment_layer: Optional[int] = None,
                                    alignment_heads: Optional[int] = None, loss_type: str = 'nmt'):
        """
        ...
        for idx, layer in enumerate(self.layers):
            ...
            x, layer_attn, _, layer_mask = layer(x, encoder_out["encoder_out"][0] if encoder_out is not None else None,
                                                 encoder_out["encoder_padding_mask"][
                                                     0] if encoder_out is not None else None,
                                                 incremental_state, self_attn_mask=self_attn_mask,
                                                 self_attn_padding_mask=self_attn_padding_mask,
                                                 need_attn=bool((idx == alignment_layer)),
                                                 need_head_weights=bool((idx == alignment_layer)),
                                                 attacker=self.attacker[idx], loss_type=loss_type)
        ...
        return x, {"attn": [attn], "inner_states": inner_states, "mask": [mask]}


class RGFIXDecoderLayer(TransformerDecoderLayer):
    ...
    def forward(self, x, encoder_out: Optional[torch.Tensor] = None,
                ...
                need_attn: bool = False, need_head_weights: bool = False, attacker=None, loss_type: str = 'nmt'):
        """
        Args:
        ...
        if self.cross_self_attention and not (incremental_state is not None and _self_attn_input_buffer is not None
        ...
        if self.encoder_attn is not None:
            ...
            x, attn = self.encoder_attn(query=x, key=encoder_out, value=encoder_out,
                                        ...
                                        need_head_weights=need_head_weights, mask=mask, loss_type=loss_type)
        ...
        return x, attn, None, mask

    def build_encoder_attention(self, embed_dim, args):
        return AttackMultiheadAttention(embed_dim, args.decoder_attention_heads,
...
class AttackMultiheadAttention(MultiheadAttention):
    ...
    def forward(self, query, key: Optional[Tensor], value: Optional[Tensor],
                ...
                need_head_weights: bool = False, mask=None, loss_type: str = 'nmt') -> Tuple[Tensor, Optional[Tensor]]:
        ...
        if (not self.onnx_trace and not is_tpu  # don't use PyTorch version on TPUs
        ...
        if self.add_zero_attn:
            ...
            if key_padding_mask is not None:
                key_padding_mask = torch.cat([key_padding_mask, torch.zeros(
                    key_padding_mask.size(0), 1).type_as(key_padding_mask)], dim=1)
        ...
        if loss_type == 'nmt':
            attn_weights_float = attn_weights_float + 0.1 * torch.mul(attn_weights_float, torch.exp(1 - mask))
        elif loss_type == 'mask':
