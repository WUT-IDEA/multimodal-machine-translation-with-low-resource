...
fairseq-generate ${ACMMT_ROOT}/data_bin/$SRC-$TGT/${TEST} --user-dir ${GATING_ROOT} --batch-size 128 \
        --criterion my_label_smoothed_cross_entropy --task attack_translation_task \
        --path ${MODEL_PATH}  --remove-bpe --nbest 1 --num-workers 12 \
        --source-lang ${SRC} --target-lang ${TGT} --beam 8 \
        --results-path ${SAVE_DIR}/${TEST} #--lenpen 0.8 