...
@dataclass
class MyLabelSmoothedCrossEntropyCriterion(FairseqDataclass):

    mask_loss_weight: float = field(default=0., metadata={"help": 'weight of mask loss'})
    label_smoothing: float = field(default=0., metadata={"help": 'epsilon for label smoothing, 0 means no label'
                                                                 ' smoothing'})
    sentence_avg: bool = II("optimization.sentence_avg")


@register_criterion('my_label_smoothed_cross_entropy', dataclass=MyLabelSmoothedCrossEntropyCriterion)
class MyLabelSmoothedCrossEntropyCriterion(LabelSmoothedCrossEntropyCriterion):

    def __init__(self, task, sentence_avg, label_smoothing, mask_loss_weight):
    ...
    def forward(self, model, sample, reduce=True, show=False):
        ...
        # function 6 in the original paper
        p_norm = torch.norm(1 - net_output[-1]["mask"][0], p=2) / src_len
        # function 5 in the original paper
        mask_loss_final = -mask_loss + self.mask_loss_weight * p_norm

        logging_output = {"loss": loss.data, "mask_loss": mask_loss.data, "p2": p_norm.data,
                          "nll_loss": nll_loss.data, 'mask_ave': mask_ave.data,
                          "ntokens": sample["ntokens"], "nsentences": sample["target"].size(0),
                          "sample_size": sample_size, "new_weight": model.get_weight()}
