...
class TransformerModel(nn.Module):
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None,
            image_langs=None, cache=None, img_boxes=None, img_feats=None, img_mask_pos=None, enc_mask=None):
        """
        Inputs:
        ...
        if img_feats is not None and image_langs is not None:
            ...
            if img_mask_pos is not None and self.reg_mask_type == 'mask':
                ...
                # masked_scatter fills from source tensor in an ordered way expand the
                # embedding vector into a matrix, so that it always fetches the embedding
                img_feats.masked_scatter_(img_mask_pos, mask_emb.expand(int(img_mask_pos.sum()), -1))
        ...
        if self.is_encoder and self.select_attn and img_feats is not None and image_langs is not None:
            select, _ = self.selective_attn(tensor, img_attn_mask, kv=feats, cache=cache)
            concat = torch.cat((select, tensor), dim=-1)
            gate = torch.sigmoid(self.gate(concat))
            fused = (1 - gate) * tensor + gate * select
            tensor = fused + tensor
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor = self.forward('fwd', x=generated[:cur_len], lengths=gen_len,
            ...
            assert tensor.size() == (1, bs, self.dim), (cur_len, max_len, src_enc.size(), tensor.size(),
                                                        (1, bs, self.dim))
