...
class Evaluator(object):
    ...
    def evaluate_vmlm(self, scores, data_set, lang1, lang2):
        ...
        for batch in self.get_iterator_vmlm(data_set, lang1, lang2, stream=(lang2 is None)):
            ...
            # cuda
            x, y, pred_mask, lengths, positions, langs, image_langs, img_boxes, img_feats = to_cuda(
                x, y, pred_mask, lengths, positions, langs, image_langs, img_boxes, img_feats)

            # forward / loss
            tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, image_langs=image_langs,
                           causal=False, img_boxes=img_boxes, img_feats=img_feats)
    ...
    def evaluate_mmt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            if params.inputs_concat:
                src_len = len1 + params.num_of_regions
            # decode target sentence
            dec2 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True,
                           src_enc=enc1, src_len=src_len)
            ...
            if eval_bleu:
                ...
                if params.beam_size == 1:
                    generated, lengths = decoder.generate(enc1, src_len,
                                                          lang2_id, max_len=max_len)
                else:
                    generated, lengths, n_generated, n_lengths = decoder.generate_beam(
                        enc1, src_len, lang2_id, beam_size=params.beam_size,
                        length_penalty=params.length_penalty,
