...
def average_checkpoints(inputs):
    ...
    for fpath in inputs:
        ...
        if new_state is None:
            new_state = state
        print('1111111111111111', state.keys())
        model_params = state["encoder"]
    ...
    new_state["encoder"] = averaged_params
    return new_state
...
def last_n_checkpoints(paths, n, update_based, upper_bound=None):
    ...
    path = paths[0]
    if update_based:
        pt_regexp = re.compile(r"checkpoint_\d+_(\d+)\.pth")
    else:
        pt_regexp = re.compile(r"checkpoint(\d+)\.pth")
...
def main():
    parser = argparse.ArgumentParser(description="Tool to average the params of input checkpoints to "
    ...
    num_group.add_argument('--num-update-checkpoints', type=int,
                           help='if set, will try to find checkpoints with names checkpoint_ee_xx.pt in the path '
                                'specified by input, and average last this many of them.')
    parser.add_argument('--checkpoint-upper-bound', type=int,
                        help='when using --num-epoch-checkpoints, this will set an upper bound on which epoch to use, '
                             ...
                             'would be averaged. e.g., with --num-update-checkpoints=10 --checkpoint-upper-bound=50000,'
                             ' checkpoints 40500-50000 would  be averaged assuming --save-interval-updates 500')
    # fmt: on
