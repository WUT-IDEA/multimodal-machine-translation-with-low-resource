...
class Trainer(object):
    ...
    def vmlm_step(self, lang1, lang2, lambda_coeff, iter):
        ...
        # generate batch / select words to predict
        x, lengths, positions, langs, image_langs, img_boxes, img_feats, img_labels, _ = \
        ...
        img_boxes, img_feats, img_y, img_pred_mask, img_mask_pos = self.mask_out_image(img_boxes, img_feats, img_labels)

        # cuda
        x, y, txt_pred_mask, lengths, positions, langs, image_langs, img_boxes, img_feats, img_labels, = to_cuda(
            x, y, txt_pred_mask, lengths, positions, langs, image_langs, img_boxes, img_feats, img_labels)
        ...
        tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False,
                       image_langs=image_langs, img_boxes=img_boxes, img_feats=img_feats, img_mask_pos=img_mask_pos)
...
class EncDecTrainer(Trainer):
    ...
    def mmt_step(self, lang1, lang2, lambda_coeff):
        ...
        # generate batch
        if lang1 == lang2:
            (x1, len1), (img_boxes, img_feats, img_labels), _ = self.get_batch_vpara('vae', lang1)
            (x2, len2) = (x1, len1)
            (x1, len1) = self.add_noise(x1, len1)
        else:
            if lang1 > lang2:
        ...
        # decode target sentence
        dec2 = self.decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1,
                            src_len=src_len)
    ...
    def vbt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        # generate source batch
        (x1, len1), (img_boxes, img_feats, img_labels), _ = self.get_batch_vpara('vbt', lang1)

        langs1 = x1.clone().fill_(lang1_id)
        img_langs = torch.empty((params.num_of_regions, langs1.size(1))).long().fill_(img_id)

        # cuda
        x1, len1, langs1, img_langs, img_boxes, img_feats = to_cuda(x1, len1, langs1, img_langs,
                                                                    img_boxes, img_feats)
        ...
        with torch.no_grad():
            ...
            src_len1 = len1
            if params.inputs_concat:
                src_len1 = len1 + params.num_of_regions
            x2, len2 = _decoder.generate(enc1, src_len1, lang2_id,
                                         max_len=int(1.3 * len1.max().item() + 5))
            langs2 = x2.clone().fill_(lang2_id)
        ...
        src_len = len2
        if params.inputs_concat:
            src_len = len2 + params.num_of_regions
        dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2,
                            src_len=src_len)
