...
class Evaluator(object):
        ...
        if lang2 is None:
            if stream:
                ...
                if 'vmono' in self.data.keys():
                    ...
                    raise RuntimeError('There is no vmono or mono in data.')

                iterator = data[lang1][data_set].get_iterator(shuffle=False, group_by_size=True,
            ...
            if 'vpara' in self.data.keys():
                ...
                raise RuntimeError('There is no vpara or para in data.')
            iterator = data[(_lang1, _lang2)][data_set].get_iterator(shuffle=False, group_by_size=True,
                                                                     n_sentences=n_sentences)

        for batch in iterator:
            yield batch if lang2 is None or lang1 < lang2 else batch[::-1]

    def get_iterator_vmlm(self, data_set, lang1, lang2=None, stream=False):
        """
        Create a new iterator for a dataset.
        """
        assert data_set in ['valid', 'test']
        ...
        if lang2 is None:
            iterator = self.data['vmono'][lang1][data_set].get_iterator(shuffle=False, group_by_size=False,
                                                                        n_sentences=n_sentences)
        else:
            assert stream is False
            _lang1, _lang2 = (lang1, lang2) if lang1 < lang2 else (lang2, lang1)
            iterator = self.data['vpara'][(_lang1, _lang2)][data_set].get_iterator(shuffle=False, group_by_size=True,
                                                                                   n_sentences=n_sentences)
    ...
    def create_reference_files(self):
        ...
        for (lang1, lang2), v in data.items():
            ...
            for data_set in ['valid', 'test']:
                ...
                img_name_path = os.path.join(params.image_names, '{0}.order'.format(data_set))
                ref_img_path = os.path.join(params.hyp_path, 'img.{0}-{1}.{2}.order'.format(lang1, lang2, data_set))
                ...
                # convert to text
                if params.mmt_steps or params.vbt_steps or params.vmass_steps:
                    for (sent1, len1), (sent2, len2), _, sentences_id in self.get_iterator(data_set, lang1, lang2):
                        lang1_txt.extend(convert_to_text(sent1, len1, self.dico, params))
                        lang2_txt.extend(convert_to_text(sent2, len2, self.dico, params))
                        sent_id.extend(sentences_id)
                else:
                ...
                # output image name according to changed image order
                if not params.image_names == "":
                    ...
                    with open(ref_img_path, 'w', encoding='utf-8') as f:
                        for i in sent_id:
        ...
        with torch.no_grad():
            ...
            for data_set in ['valid', 'test']:
                ...
                for lang1, lang2 in params.mlm_steps:
                    if params.eval_vmlm:
                        self.evaluate_vmlm(scores, data_set, lang1, lang2)
                    else:
                ...
                for lang in params.vmass_steps:
                    self.evaluate_vmass(scores, data_set, lang)
                ...
                vmass_steps = []
                for lang1 in params.vmass_steps:
                    for lang2 in params.vmass_steps:
                        if lang1 != lang2:
                            vmass_steps.append((lang1, lang2))
                ...
                for lang1, lang2 in set(params.mmt_steps + [(l2, l3) for _, l2, l3 in params.vbt_steps] + vmass_steps):
                    eval_bleu = params.eval_bleu and params.is_master
                ...
                if len(_mlm_mono) > 0:
                    ...
                    if params.eval_vmlm:
                        scores['%s_vmlm_ppl' % data_set] = np.mean(
                            [scores['%s_%s_vmlm_ppl' % (data_set, lang)] for lang in _mlm_mono])
                        scores['%s_vmlm_acc' % data_set] = np.mean(
                            [scores['%s_%s_vmlm_acc' % (data_set, lang)] for lang in _mlm_mono])
    ...
    def evaluate_vmlm(self, scores, data_set, lang1, lang2):
        """
        Evaluate perplexity and next word prediction accuracy.
        """
        params = self.params
        ...
        for batch in self.get_iterator_vmlm(data_set, lang1, lang2, stream=(lang2 is None)):

            # batch
            if lang2 is not None:
                # VTLM
                _, (img_boxes, re_feats, gr_feats, gl_feats), (x1, len1), (x2, len2) = batch
                ...
                # VMLM
                (x, len1), (img_boxes, re_feats, gr_feats, gl_feats), _ = batch
            ...
            elif params.eval_probes.starstwith('drop_last:'):
                # drop last words
            ...
            if params.re_img_feats and not params.gr_img_feats and not params.gl_img_feats:
                img_langs = torch.empty((params.num_of_regions, langs.size(1))).long().fill_(img_id)
            elif params.gr_img_feats and not params.re_img_feats and not params.gl_img_feats:
                img_langs = torch.empty((49, langs.size(1))).long().fill_(img_id)
            elif params.gl_img_feats and not params.gr_img_feats and not params.re_img_feats:
                img_langs = torch.empty((1, langs.size(1))).long().fill_(img_id)
            elif params.re_img_feats and params.gr_img_feats and not params.gl_img_feats:
                img_langs = torch.empty((params.num_of_regions + 49, langs.size(1))).long().fill_(img_id)
            elif params.re_img_feats and params.gl_img_feats and not params.gr_img_feats:
                img_langs = torch.empty((params.num_of_regions + 1, langs.size(1))).long().fill_(img_id)
            elif params.gr_img_feats and params.gl_img_feats and not params.re_img_feats:
                img_langs = torch.empty((49 + 1, langs.size(1))).long().fill_(img_id)
            elif params.re_img_feats and params.gr_img_feats and params.gl_img_feats:
                img_langs = torch.empty((params.num_of_regions + 49 + 1, langs.size(1))).long().fill_(img_id)
            else:
                raise Exception("There is no other case")

            # cuda
            x, y, pred_mask, lengths, positions, langs, img_boxes, re_feats, gr_feats, image_langs, gl_feats = to_cuda(
                x, y, pred_mask, lengths, positions, langs, img_boxes, re_feats, gr_feats, img_langs, gl_feats)

            # forward / loss
            tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, image_langs=image_langs,
                           causal=False, img_boxes=img_boxes, re_img_feats=re_feats, gr_img_feats=gr_feats,
                           gl_img_feats=gl_feats)

            # Fetch linguistic part of the hidden states for accuracy computation
            if params.visual_first:
                sent_tensor = tensor[image_langs.size(0):]
            else:
        ...
        # compute perplexity and prediction accuracy
        ppl_name = '%s_%s_vmlm_ppl' % (data_set, l1l2)
        acc_name = '%s_%s_vmlm_acc' % (data_set, l1l2)
        scores[ppl_name] = np.exp(xe_loss / n_words) if n_words > 0 else 1e9
        scores[acc_name] = 100. * n_valid / n_words if n_words > 0 else 0.
...
class EncDecEvaluator(Evaluator):
    ...
    def evaluate_vmass(self, scores, data_set, lang):
        params = self.params
        assert data_set in ['valid', 'test']
        assert lang in params.langs
        ...
        for batch in self.get_iterator(data_set, lang):

            (x1, len1), (img_boxes, img_feats, img_labels), _ = batch
            (x1, len1, x2, len2, y, pred_mask, positions) = self.mask_sent(x1, len1, rng)
            ...
            # cuda
            x1, len1, langs1, x2, len2, langs2, y, positions = to_cuda(x1, len1, langs1, x2, len2, langs2, y, positions)
            image_langs, img_boxes, img_feats = to_cuda(image_langs, img_boxes, img_feats)

            # encode source sentence
            enc1 = encoder('fwd', x=x1, lengths=len1, langs=langs1, causal=False, image_langs=image_langs,
        ...
        # compute perplexity and prediction accuracy
        scores['%s_%s-%s_vmass_ppl' % (data_set, lang, lang)] = np.exp(xe_loss / n_words)
        scores['%s_%s-%s_vmass_acc' % (data_set, lang, lang)] = 100. * n_valid / n_words
    ...
    def evaluate_mt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            ...
            if eval_bleu:
                ...
                if params.beam_size == 1:
                    ...
                    generated, lengths, n_generated, n_lengths = decoder.generate_beam(
                ...
                if params.nbest:
                    n_hypothesis.append(convert_to_text(n_generated, n_lengths, self.dico, params))
        ...
        if eval_bleu:
            ...
            if params.nbest:
                n_hyp_name = 'nbest_hypo{0}.{1}-{2}.{3}.txt'.format(scores['epoch'], lang1, lang2, data_set)
                n_hyp_path = os.path.join(params.hyp_path, n_hyp_name)
                # export sentences to hypothesis file / restore BPE segmentation
                with open(n_hyp_path, 'w', encoding='utf-8') as f:
                    for i in range(len(n_hypothesis)):
                        f.write('\n'.join(n_hypothesis[i]) + '\n')
                restore_segmentation(n_hyp_path)
    ...
    def evaluate_mmt(self, scores, data_set, lang1, lang2, eval_bleu):
        ...
        for batch in self.get_iterator(data_set, lang1, lang2):
            # generate batch
            if lang1 > lang2:
                _, (img_boxes, re_img_feats, gr_img_feats, gl_img_feats), (x1, len1), (x2, len2) = batch
            else:
                (x1, len1), (x2, len2), (img_boxes, re_img_feats, gr_img_feats, gl_img_feats), _ = batch
            ...
            x1, len1, langs1, x2, len2, langs2, y, img_langs, img_boxes, re_img_feats, gr_img_feats, gl_img_feats, src_len\
                = to_cuda(x1, len1, langs1, x2, len2, langs2, y, img_langs, img_boxes, re_img_feats, gr_img_feats,
                          gl_img_feats, src_len)

            # encode source sentence
            enc1 = encoder('fwd', x=x1, lengths=len1, langs=langs1, image_langs=img_langs, causal=False,
            ...
            # decode target sentence
            dec2 = decoder('fwd', x=x2, lengths=len2, langs=langs2, causal=True, src_enc=enc1, src_len=src_len)
            ...
            if eval_bleu:
                ...
                if params.beam_size == 1:
                    generated, lengths = decoder.generate(enc1, src_len, lang2_id, max_len=max_len)
                else:
                    generated, lengths, n_generated, n_lengths = decoder.generate_beam(
                        enc1, src_len, lang2_id, beam_size=params.beam_size, length_penalty=params.length_penalty,
                        early_stopping=params.early_stopping, max_len=max_len)
        ...
        # compute perplexity and prediction accuracy
        scores['%s_%s-%s_mmt_ppl' % (data_set, lang1, lang2)] = np.exp(xe_loss / n_words)
        scores['%s_%s-%s_mmt_acc' % (data_set, lang1, lang2)] = 100. * n_valid / n_words
