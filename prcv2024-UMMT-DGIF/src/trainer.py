...
class Trainer(object):
    ...
    def generate_batch_vpara(self, lang1, lang2, name):
        ...
        if lang2 is not None:
            # VTLM
            _, (boxes, re_feats, gr_feats, gl_feats), (x1, len1), (x2, len2) = self.get_batch_vpara(name, lang1, lang2)
            ...
            # VMLM
            (x, len1), (boxes, re_feats, gr_feats, gl_feats), _ = self.get_batch_vpara(name, lang1, lang2)
            langs = x.clone().fill_(lang1_id)
            lengths = len1
            positions = None
        return x, lengths, positions, langs,  boxes, re_feats, gr_feats, gl_feats,(None, None)
    ...
    def vmlm_step(self, lang1, lang2, lambda_coeff, iter):
        ...
        # generate batch / select words to predict
        x, lengths, positions, langs, img_boxes, re_img_feats, gr_img_feats, gl_img_feats, _ = \
        ...
        x, y, txt_pred_mask, lengths, positions, langs, image_langs, img_boxes, re_img_feats, gr_img_feats, gl_img_feats= (
            to_cuda(x, y, txt_pred_mask, lengths, positions, langs, img_langs, img_boxes, re_img_feats,
                    gr_img_feats, gl_img_feats))
        ...
        tensor = model('fwd', x=x, lengths=lengths, positions=positions, langs=langs, causal=False,
                       ...
                       gr_img_feats=gr_img_feats, gl_img_feats=gl_img_feats # img_mask_pos=img_mask_pos
                       )

        # slice the outputs back to feed into prediction heads
        if params.visual_first:
            sent_tensor = tensor[image_langs.size(0):]
            img_tensor = tensor[:image_langs.size(0)]
        else:
...
class EncDecTrainer(Trainer):
    ...
    def vbt_step(self, lang1, lang2, lang3, lambda_coeff):
        ...
        if params.re_img_feats and not params.gr_img_feats and not params.gl_img_feats:
            len_change = params.num_of_regions
            img_langs = torch.empty((params.num_of_regions, langs1.size(1))).long().fill_(img_id)
        elif params.gr_img_feats and not params.re_img_feats and not params.gl_img_feats:
            len_change = 49
            img_langs = torch.empty((49, langs1.size(1))).long().fill_(img_id)
            ...
            len_change = params.num_of_regions + 49
            img_langs = torch.empty((params.num_of_regions + 49, langs1.size(1))).long().fill_(img_id)
        elif params.re_img_feats and params.gl_img_feats and not params.gr_img_feats:
            len_change = params.num_of_regions + 1
            img_langs = torch.empty((params.num_of_regions + 1, langs1.size(1))).long().fill_(img_id)
        elif params.gr_img_feats and params.gl_img_feats and not params.re_img_feats:
            len_change = 49 + 1
            img_langs = torch.empty((49 + 1, langs1.size(1))).long().fill_(img_id)
        elif params.re_img_feats and params.gr_img_feats and params.gl_img_feats:
            len_change = params.num_of_regions + 49 + 1
            img_langs = torch.empty((params.num_of_regions + 49 + 1, langs1.size(1))).long().fill_(img_id)
        else:
            raise Exception("There is no other case")

        # cuda
        x1, len1, langs1, img_langs, img_boxes, re_img_feats, gr_img_feats, gl_img_feats = to_cuda(
            x1, len1, langs1, img_langs,img_boxes, re_img_feats, gr_img_feats, gl_img_feats)
        ...
        # decode original sentence
        dec3 = self.decoder('fwd', x=x1, lengths=len1, langs=langs1, causal=True, src_enc=enc2, src_len=src_len2)
