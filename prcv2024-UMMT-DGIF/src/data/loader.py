...
def read_image_names(root, split, lang=None):
    image_names, existing_indices = [], []
    if lang is not None:
        split = f"{split}-{lang}.order"
    else:
        split = f"{split}.order"
...
def process_binarized(data, params):
    ...
    logger.info("%i words (%i unique) in %i sentences. %i unknown words (%i unique) covering %.2f%% of the data." % (
        len(data['sentences']) - len(data['positions']), len(dico), len(data['positions']),
...
def load_vmono_data(params, data):
    """
    Load monolingual data with image features
    """
    data['vmono'] = {}
    ...
    for lang in params.mono_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            # image features
            existing_indices, image_names = read_image_names(params.image_names, splt, lang)
            existing_indices = np.array(existing_indices)
            logger.info(f'Found {existing_indices.size} image features')
            # for de-noising auto-encoding and online back-translation, we need a non-stream (batched) dataset
            # create stream dataset
            dataset = DatasetWithRegions(mono_data['sentences'], mono_data['positions'], image_names, params)
...
def load_para_data(params, data):
    ...
    for src, tgt in params.para_dataset.keys():
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            dataset = ParallelDataset(src_data['sentences'], src_data['positions'],
                                      tgt_data['sentences'], tgt_data['positions'], params)
...
def load_vpara_data(params, data):
    """
    Load parallel data along with image features.
    """
    data['vpara'] = {}

    required_para_train = set(params.clm_steps + params.mlm_steps + params.pc_steps + params.mt_steps +
                              params.mmt_steps + params.vbt_steps)
    
    for src, tgt in params.para_dataset.keys():

        logger.info('============ Parallel data (%s-%s) =============' % (src, tgt))

        assert (src, tgt) not in data['vpara']
        data['vpara'][(src, tgt)] = {}
        ...
        for splt in ['train', 'valid', 'test']:
            ...
            src_data = load_binarized(src_path, params)
            tgt_data = load_binarized(tgt_path, params)
            existing_indices, image_names = read_image_names(params.image_names, splt)
            ...
            dataset = ParallelDatasetWithRegions(src_data['sentences'], src_data['positions'],
                                                 tgt_data['sentences'], tgt_data['positions'], image_names, params)
            ...
            data['vpara'][(src, tgt)][splt] = dataset
            logger.info("")
...
def check_data_params(params):
    ...
    vmlm_steps = [s.split('-') for s in params.vmlm_steps.split(',') if len(s) > 0]
    params.mlm_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in mlm_steps]
    params.vmlm_steps = [(s[0], None) if len(s) == 1 else tuple(s) for s in vmlm_steps]
    ...
    params.vae_steps = [s for s in params.vae_steps.split(',') if len(s) > 0]
    ...
    params.vmass_steps = [s for s in params.vmass_steps.split(',') if len(s) > 0]
    ...
    for src in params.vmass_steps:
        for tgt in params.vmass_steps:
            if src != tgt:
                vmass_steps.append(tuple([src, tgt]))

    # back-translation steps
    params.bt_steps = [tuple(s.split('-')) for s in params.bt_steps.split(',') if len(s) > 0]
    params.vbt_steps = [tuple(s.split('-')) for s in params.vbt_steps.split(',') if len(s) > 0]
    ...
    assert all([len(x) == 3 for x in params.vbt_steps])
    assert all([l1 in params.langs and l2 in params.langs and l3 in params.langs for l1, l2, l3 in params.vbt_steps])
    assert all([l1 == l3 and l1 != l2 for l1, l2, l3 in params.vbt_steps])
    assert len(params.vbt_steps) == len(set(params.vbt_steps))
    assert len(params.vbt_steps) == 0 or not params.encoder_only

    params.bt_src_langs = [l1 for l1, _, _ in params.bt_steps]
    params.vbt_src_langs = [l1 for l1, _, _ in params.vbt_steps]

    # check monolingual datasets
    required_mono = set([l1 for l1, l2 in (params.mlm_steps + params.vmlm_steps + params.clm_steps
                                           ) if l2 is None] + params.ae_steps + params.bt_src_langs +
                        params.vae_steps + params.vbt_src_langs + params.mass_steps + params.vmass_steps)

    params.mono_dataset = {
        lang: {splt: os.path.join(params.data_path, '%s.%s.pth' % (splt, lang))
               for splt in ['train', 'valid', 'test']} for lang in params.langs if lang in required_mono}
    ...
    required_para |= set([(l2, l3) for _, l2, l3 in params.vbt_steps] + mass_steps + vmass_steps)
    ...
    assert params.eval_bleu is False or len(params.mt_steps + params.bt_steps +
                                            params.mmt_steps + params.vbt_steps + mass_steps + vmass_steps) > 0
...
def load_data(params):
    ...
    if params.mmt_steps or params.vbt_steps or params.only_vmlm or params.vmass_steps:
        if params.only_vmlm or params.vbt_steps or params.vmass_steps:
    ...
    if "vmono" in data:
        for lang, v in data['vmono'].items():
            for data_set in v.keys():
                logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.
                            format('Monolingual data with visual features', data_set, lang, len(v[data_set])))
    ...
    if "para" in data:
        for (src, tgt), v in data['para'].items():
            for data_set in v.keys():
                logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.
                            format('Parallel data', data_set, '%s-%s' % (src, tgt), len(v[data_set])))

    # vpara data summary
    if "vpara" in data:
        for (src, tgt), v in data['vpara'].items():
            for data_set in v.keys():
                logger.info('{: <18} - {: >5} - {: >12}:{: >10}'.
                            format('Parallel data with visual features', data_set, '%s-%s' % (src, tgt), len(v[data_set])))
