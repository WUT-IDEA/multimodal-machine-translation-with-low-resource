...
from .transformer import DECODER_ONLY_PARAMS, TransformerModel, HashingMemory  # , TRANSFORMER_LAYER_PARAMS
...
def check_model_params(params):
    ...
    if len(params.ae_steps) == 0 and len(params.vae_steps) == 0:
...
def set_pretrain_emb(model, dico, word2id, embeddings):
    ...
    logger.info("Pretrained %i/%i words (%.3f%%)." % (n_found, len(dico), 100. * n_found / len(dico)))
...
def build_model(params, dico):
    ...
    if params.encoder_only:
        ...
        # reload a pretrained model
        # params.reload_model = "/data1/home/turghun/project/VMLM/pretrained/en-de/XLM/best-valid_mlm_ppl.pth"
        if params.reload_model != '':
