...
DECODER_ONLY_PARAMS = ['layer_norm15.%i.weight', 'layer_norm15.%i.bias',
...
TRANSFORMER_LAYER_PARAMS = ['attentions.%i.q_lin.weight', 'attentions.%i.q_lin.bias',
...
class ImgPredLayer(nn.Module):
    ...
    def __init__(self, params):
        ...
        if params.asm is False:
            ...
            self.proj = nn.AdaptiveLogSoftmaxWithLoss(in_features=dim, n_classes=params.num_obj_labels,
...
class PredLayer(nn.Module):
    ...
    def __init__(self, params):
        ...
        if params.asm is False:
            ...
            self.proj = nn.AdaptiveLogSoftmaxWithLoss(in_features=dim, n_classes=params.n_words,
                                                      cutoffs=params.asm_cutoffs, div_value=params.asm_div_value,
                                                      head_bias=True,  # default is False
            )
...
class Projector(nn.Module):
    ...
    def __init__(self, params):
        ...
        self.re_linear = nn.Linear(1536, params.emb_dim)
        self.gr_linear = nn.Linear(2048, params.emb_dim)
        #self.gl_linear = nn.Linear(2048, params.emb_dim)

    def forward(self, x):
        if x.size(1) == 36:
            x = self.re_linear(x.view(x.size(0), x.size(1), -1))
        elif x.size(1) == 49:
            x = self.gr_linear(x.view(x.size(0), x.size(1), -1))
        # elif x.size(1) == 1:
        #     x = self.gl_linear(x.view(x.size(0), x.size(1), -1))
...
class RegionalEncodings(nn.Module):
    ...
    def __init__(self, params):
        super().__init__()
        self.relu = getattr(params, 'visual_relu', True)
        self.linear = nn.Linear(4, params.emb_dim, bias=getattr(params, 'reg_enc_bias', True))
...
class TransformerModel(nn.Module):
    ...
    def __init__(self, params, dico, is_encoder, with_output):
        ...
        self.regional_encodings = RegionalEncodings(params)
        self.inputs_concat = params.inputs_concat
        self.select_attn = params.select_attn

        self.re_feat = params.re_img_feats
        self.gr_feat = params.gr_img_feats
        self.gl_feat = params.gl_img_feats
        # model parameters
        ...
        if self.is_encoder and self.select_attn:
            self.gate = nn.Linear(2 * self.dim, self.dim)
            self.selective_attn = MultiHeadAttention(self.n_heads, self.dim, dropout=self.attention_dropout)
        # memories
        ...
        if self.with_output:
            ...
            self.img_pred_layer = ImgPredLayer(params)
    ...
    def fwd(self, x, lengths, causal, src_enc=None, src_len=None, positions=None, langs=None,
            image_langs=None, cache=None, img_boxes=None, re_img_feats=None, gr_img_feats=None, gl_img_feats=None,
            img_mask_pos=None, enc_mask=None):
        """
        Inputs:
        ...
        if re_img_feats is not None or gr_img_feats is not None or gl_img_feats is not None:
            gl_img_feats = gl_img_feats.unsqueeze(1)
            if self.re_feat and not self.gr_feat and not self.gl_feat: # 36
                # create masks which are always active by the way
                img_mask = mask.new_ones((mask.size(0), self.num_of_regions))
                re_img_feats = self.projector(re_img_feats)
                feats = re_img_feats + self.regional_encodings(img_boxes)

            elif self.gr_feat and not self.re_feat and not self.gl_feat: # 49
                img_mask = mask.new_ones((mask.size(0), 49))
                feats = self.projector(gr_img_feats)

            elif self.gl_feat and not self.re_feat and not self.gr_feat: # 1
                img_mask = mask.new_ones((mask.size(0), 1))
                feats = self.projector(gl_img_feats)

            elif self.re_feat and self.gr_feat and not self.gl_feat: # 36 + 49
                img_mask = mask.new_ones((mask.size(0), self.num_of_regions + 49))
                re_img_feats = self.projector(re_img_feats)
                gr_img_feats = self.projector(gr_img_feats)
                re_img_feats = re_img_feats + self.regional_encodings(img_boxes)
                feats = torch.cat((re_img_feats, gr_img_feats), dim=1)

            elif self.re_feat and self.gl_feat and not self.gr_feat: # 36 + 1
                img_mask = mask.new_ones((mask.size(0), self.num_of_regions + 1))
                re_img_feats = self.projector(re_img_feats)
                gl_img_feats = self.projector(gl_img_feats)
                re_img_feats = re_img_feats + self.regional_encodings(img_boxes)
                feats = torch.cat((re_img_feats, gl_img_feats), dim=1)

            elif self.gr_feat and self.gl_feat and not self.re_feat: # 49 + 1
                img_mask = mask.new_ones((mask.size(0), 49 + 1))
                gr_img_feats = self.projector(gr_img_feats)
                gl_img_feats = self.projector(gl_img_feats)
                feats = torch.cat((gr_img_feats, gl_img_feats), dim=1)

            elif self.re_feat and self.gr_feat and self.gl_feat: # 36 + 49 + 1
                img_mask = mask.new_ones((mask.size(0), self.num_of_regions + 49 + 1))
                re_img_feats = self.projector(re_img_feats)
                gr_img_feats = self.projector(gr_img_feats)
                gl_img_feats = self.projector(gl_img_feats)
                re_img_feats = re_img_feats + self.regional_encodings(img_boxes)
                re_gr_fuse = torch.cat((re_img_feats, gr_img_feats), dim=1)
                feats = torch.cat((re_gr_fuse, gl_img_feats), dim=1)
            else:
            ...
            if self.inputs_concat:

                # concatenate: put image sequences first of text sequence
    ...
    def predict_img_class(self, tensor, pred_mask, y, get_scores):
        ...
        """
        masked_tensor = tensor[pred_mask.unsqueeze(-1).expand_as(tensor)].view(-1, self.dim)
        scores, loss = self.img_pred_layer(masked_tensor, y, get_scores)
        return scores, loss
    ...
    def generate(self, src_enc, src_len, tgt_lang_id, max_len=200, sample_temperature=None):
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor = self.forward('fwd', x=generated[:cur_len], lengths=gen_len, positions=positions[:cur_len],
                                  langs=langs[:cur_len], causal=True, src_enc=src_enc, src_len=src_len, cache=cache)
    ...
    def generate_beam(self, src_enc, src_len, tgt_lang_id, beam_size, length_penalty, early_stopping, max_len=200):
        ...
        bs = len(src_len)
        n_words = self.n_words # vocab_size
        ...
        while cur_len < max_len:
            ...
            # compute word scores
            tensor = self.forward('fwd', x=generated[:cur_len], lengths=src_len.new(bs * beam_size).fill_(cur_len),
        ...
        n_tgt_len = src_len.new(bs * beam_size)
        ...
        for i, hypotheses in enumerate(generated_hyps):
            ...
            for n_hypotheses in hypotheses.hyp:
                j += 1
                (_, n_hypo) = n_hypotheses
                n_tgt_len[j] = len(n_hypo) + 1
                nbest.append(n_hypo)

        # generate target batch
        decoded = src_len.new(tgt_len.max().item(), bs).fill_(self.pad_index)
        n_decoded = src_len.new(n_tgt_len.max().item(), bs * beam_size).fill_(self.pad_index)

        for i, hypo in enumerate(best):
            decoded[:tgt_len[i] - 1, i] = hypo
            decoded[tgt_len[i] - 1, i] = self.eos_index

        for n, n_hypo in enumerate(nbest):
            n_decoded[:n_tgt_len[n] - 1, n] = n_hypo
            n_decoded[n_tgt_len[n] - 1, n] = self.eos_index

        # sanity check
        assert (decoded == self.eos_index).sum() == 2 * bs
        return decoded, tgt_len, n_decoded, n_tgt_len
